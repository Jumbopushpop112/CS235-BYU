...
#elif SORT == 8 // better bubble
#include "better_bubble.h"
...
bool certify_stable(vector<SideCar<T>> const &items)
{
    ...
    for (size_t i = 1; i < items.size(); i++) {
        if (items[i-1].value == items[i].value && items[i-1].pos > items[i].pos) {
            // i.e. the values are the same, 
            // but the original position of the prior item 
            // is larger than the original position of the current item
            return false;
        }
    }
    return true;
